...
// notify admin namespace (unchanged)
      //io.of("/admin").emit("server:updateGames", { totalGamesPlayed });

// ALSO notify connected game clients so they can update HTML counters
// `gameNS` is the game namespace variable above
      ...
      try {
        ...
        while (!aborted) {
          ...
          //gameNS.emit("server:updateTurns", { totalTurnsThisGame });
          //io.of("/admin").emit("server:updateTurns", { totalTurnsThisGame });
          if (round % 2 === 0) {
        ...
        //gameNS.emit("server:updateGames", { totalGamesPlayed });
        //gameNS.emit("server:updateTurns", { totalTurnsThisGame });
        //io.of("/admin").emit("server:updateTurns", { totalTurnsThisGame }); // optional for admin
