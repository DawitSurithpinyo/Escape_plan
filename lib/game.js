...
const waiting = [];
const movementBuffer = {dx: null, dy: null}
...
function gameIO(io) {
  ...
  gameNS.on("connection", async (socket) => {
    ...
    if(waiting.length === 2){
      ...
      warder.emit("start game", {grid, obsPosArray, wardenPos, prisonerPos, tunnelPos, character:"warder"});
      prisoner.emit("start game", {grid, obsPosArray, wardenPos, prisonerPos, tunnelPos, character:"prisoner"});
