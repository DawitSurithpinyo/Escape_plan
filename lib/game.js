...
var waiting = [];
var prisonerMovementBuffer = {dx: null, dy: null}
var warderMovementBuffer = {dx: null, dy: null}
    ...
    if(waiting.length === 2){
      ...
      while (!winner) {
        if (round % 2 === 0) {
          ...
          endTurn(prisoner, "prisoner", dx, dy);
 ...
 * @param {Socket} player
 */
function endTurn(player, character, dx, dy) {
  player.emit("turn end", {character, dx, dy});
